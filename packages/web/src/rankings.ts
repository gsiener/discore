...
const rows: RankRow[] = snapshot.teams.map((t) => ({
  id: t.id,
  name: t.name,
  rank: t.rank,
  rating: t.rating,
...
function renderMeta(): void {
  ...
  document.getElementById('rk-division')!.textContent = `${m.division === 'boys' ? 'High School Boys' : m.division} · ${m.season}`;
  const meta = document.getElementById('rk-meta')!;
  meta.innerHTML = '';
  const strong = document.createElement('strong');
  strong.textContent = 'Snapshot';
  meta.appendChild(strong);
  meta.append(` · ${m.totalGames} games · final results through ${fmtDate(m.resultsThrough)} · updated ${fmtDate(m.generatedAt)}`);
}
